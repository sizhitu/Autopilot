...
def get_stock_status(code: str, name: str, days: int = 300) -> StockStatus:
    ...
    try:
        ...
        # 藤本茂阶梯：近5日粗筛，阈值按持仓定位缩放（勿在函数内再 import FujimotoStrategy，会 UnboundLocalError）
        try:
            _board = getattr(FujimotoStrategy, "ROLE_BOARD_5D", None) or {
                "压舱石": (-0.12, 0.22),
                "高赔率": (-0.18, 0.30),
                "周期弹性": (-0.20, 0.32),
                "卫星仓": (-0.28, 0.40),
            }
            _b5, _s5 = _board.get(status.role, _board.get("压舱石", (-0.15, 0.25)))
